...
DATASET_PATH = "/mnt/data/backup/adam/llm_train_data/imdb_tokenized/"
...
WARMUP_STEPS = 10000 #based on BERT paper
#learning steps settings
MAX_STEPS = 25177 #1000 #10 #25177 # == total dataset size (3.3B token corpus) / total tokens per batch = 3.3e9 / 131072 = 25177 (in the paper its 40 epochs, we do 1 for start) #4
...
class IMDBDataLoader():
    def __init__(self, batch_size, token_count, sequence_length, tokenizer, process_rank, num_processes, dataset_path ="", split="train"): #amount of samples in one batch , amount of tokens per sample
...
def evaluation_step(model, val_loader, device, ddp, master_process):
    ...
    with torch.no_grad():
        ...
        for _ in range(validation_steps):
            ...
            loss_val1 = loss_fn(logits_next_stentece, y_next,ignore_index=0) #data["is_next"]
...
def format_prediction(logits, logits_next, tokenizer, sample=False, topk =1):
    topk_probs, topk_indices= torch.topk(logits, topk, dim=-1)
    if sample==True:
...
for step_no in range(MAX_STEPS):
    ...
    for micro_batch in range(ACCUM_BATCH_SIZE):
        ...
        acc = 0 #get_masked_accuracy(logits_missing_words,y) / ACCUM_BATCH_SIZE
        loss = loss_val1 + loss_val2
