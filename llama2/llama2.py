...
class Llama_Config: #default config as per GPT-2 paper, smallest GPT-2 version
    ...
    vocabulary_size: int = 32000 #llama2 vocabulry size
    embedding_size: int = 768 #embedding size
    layer_count: int = 12 # TODO: 32 in llama2 number of transformer decoder layers
    head_count: int = 12 # TODO: 32 in llama2 llama2 number of heads in self-attention for each decoder layer
    transformer_feed_forward_hidden_dim: int = 11008 #size of linear layer in the transformer block (this is used in Llama SwiGLU that replaces RELU/GELU)
...
class Llama2(nn.Module):
    def __init__(self,config):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            ln_f = nn.RMSNorm(normalized_shape=self.config.embedding_size) #nn.LayerNorm(self.config.embedding_size),
        ))
