...
def evaluation_step(model, val_loader, device, ddp, master_process):
    ...
    with torch.no_grad():
        ...
        for _ in range(validation_steps):
            ...
            loss_val2 = loss_fn(logits_missing_words.transpose(1, 2), y,ignore_index=-100)
            loss = loss_val2
...
t5_config = T5_Config()
t5_config.vocabulary_size=VOCABULARY_SIZE
t5_config.embedding_size=EMBEDDING_SIZE
t5_config.max_context_size=MAX_INPUT_LEN
t5_config.layer_count=N_LAYERS
t5_config.head_count=N_ATTN_HEADS
...
# optimizer = torch.optim.AdamW(model.parameters(), lr=5e-05, betas=(0.9, 0.999), eps=1e-8,weight_decay=0.01)
